...
key_letter='a'; %偏移量（原算法没有）

%==生成替换表======
key_count=size(key_phrase);%密钥短语长度%
origin_letter_box='abcdefghijklmnopqrstuvwxyz';%原本的字母表%
...
for i=1:count%依照字母表加密%
    E4(i)=  letter_box(M(i)-'a'+1);%======找到第i位明文M(i)所在位置对应letter_box中字母赋值给E4(i)(MATLAB中计数是从1开始，所以需要加1)=========
end
miwen = E4
